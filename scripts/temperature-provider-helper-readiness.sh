...
CAPTURE_TIMEOUT_SECONDS=${AGENTWAKE_TEMPERATURE_HELPER_READINESS_TIMEOUT_SECONDS:-1}
if ! [[ "$CAPTURE_TIMEOUT_SECONDS" =~ ^[0-9]+$ ]] || [[ "$CAPTURE_TIMEOUT_SECONDS" -le 0 ]]; then
    echo "AGENTWAKE_TEMPERATURE_HELPER_READINESS_TIMEOUT_SECONDS must be a positive integer" >&2
    exit 64
fi
