...
verify_config() {
    ...
    if ! is_choice_value "$CONFIG_PROVIDER_SOURCE" "powermetrics" "SMC" "IOReport" "other"; then
        record_error "validation-config.txt" "providerSource must be powermetrics, SMC, IOReport, or other"
    fi
...
verify_manual_result() {
    ...
    check_choice_field "Provider source" "powermetrics" "SMC" "IOReport" "other"
