...
As of the May 14, 2026 owner decision, the split readiness/prototype issues are
no longer separate execution blockers. Keep the evidence below as carry-forward
context for final app E2E validation in #120:
...
samples, and provider fail-closed states. Final E2E still needs scale/coverage
evidence before broad hardware claims, but those fields are no longer an arm-time
gate. The following support slices are now covered on `main`:
...
| Provider and safety gate | Safety policy fail-closed behavior is covered. Runtime release is wired for configured battery floor, direct IOReport temperature cutoff when samples are usable, and provider failure states. | Live provider scale, freshness, cadence, closed-bag coverage, timeout behavior, and final provider verifier success before broad direct-temperature hardware claims. |
...
`pmset disablesleep` is the local/admin-approved primitive used by the current
Closed-Lid Mode toggle. It is still treated as not verifier-complete for the
broader automated helper/safety matrix. Start validation with a baseline-only
capture:
...
| Helper lifecycle carried into #120 | helper restart, helper upgrade mid-hold |
...
replacement, production repair conflicts, and helper-owned Closed-Lid Mode cleanup
remain open.
...
Closed-Lid Mode state cleanup, and optional cask/package evidence. CLI helper
...
cutoff evidence; final app E2E must promote a better helper-owned source or
...
Issue #7 is closed because every failed or inconclusive primitive, helper,
temperature, or Codex assumption now has either:
