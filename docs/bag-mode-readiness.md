...
|---|---|---|
| [#29](https://github.com/makeavish/ClawShell/issues/29) primitive matrix | `.build/power-validation/bag-mode-matrix/apple-silicon-ac-internal-20260513T115058Z` is verified single-case Apple Silicon AC/internal evidence. The harness applied `/usr/bin/pmset disablesleep 1`, observed `SleepDisabled 1`, rolled back to `0`, and the operator reported reopen recovery `yes`; lid-close sleep-block result was `inconclusive`, so the primitive remains unproven. | Continue the primitive matrix on target hardware: battery, other physically available display states, open/closed/reopen paths, app quit/crash, and reboot-held lifecycle evidence. |
