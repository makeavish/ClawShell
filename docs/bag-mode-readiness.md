...
This captures SMC, PMU temperature sensor, die temperature controller, HID
service, and IOReport-style discovery evidence. It separates accepted numeric
candidates from rejected battery-context candidates. PMU `tdev`/`tdie` names in
HID output are inventory only. The probe does not prove helper ownership or
