...
public struct BagModeSafetyDiagnostic: Equatable, Sendable {
    ...
    public static func userFacing(for decision: BagModeSafetyDecision) -> BagModeSafetyDiagnostic? {
        ...
        case .warn:
            return BagModeSafetyDiagnostic(
                title: "Bag Mode warning: thermal conditions are elevated",
                detail: "ClawShell can keep holding sleep, but current thermal signals need attention.",
        ...
        case .failClosedBeforeArming, .releaseIfArmed:
            guard let reason = decision.state.cutoffReason else {
                return BagModeSafetyDiagnostic(
                    ...
                    detail: "ClawShell could not confirm that Bag Mode is safe to use right now.",
    ...
    private static func titlePrefix(for action: BagModeSafetyAction) -> String {
        switch action {
        case .releaseIfArmed:
            return "Bag Mode released: "
        case .failClosedBeforeArming:
            return "Bag Mode unavailable: "
        case .allow, .warn:
            return ""
        }
    }
...
private extension BagModeSafetyCutoffReason {
    ...
    var detail: String {
        switch self {
        case .temperature:
            return "The temperature cutoff was reached, so ClawShell will not keep Bag Mode armed."
        case .battery:
            return "Battery is at or below the configured floor, so ClawShell will not keep Bag Mode armed."
        case .staleSensor:
            return "The last temperature sample is too old to trust for closed-lid operation."
        case .unavailableSensor:
            return "ClawShell cannot read the temperature provider required for Bag Mode."
        case .permissionDenied:
            return "macOS denied access to the temperature provider required for Bag Mode."
        case .parseFailed:
            return "ClawShell received provider output but could not turn it into a trusted temperature reading."
        case .helperCrashed:
            return "The helper process stopped before ClawShell could confirm the Bag Mode safety state."
        case .unsupportedHardware:
            return "This Mac does not expose the validated signals ClawShell requires for Bag Mode."
        case .timedOut:
        ...
        case .batteryUnavailable:
            return "ClawShell cannot read the battery level required for Bag Mode."
        case .batteryInvalid:
    ...
    var recoveryAction: String {
        switch self {
        case .temperature:
            return "Let the Mac cool before re-arming Bag Mode."
        case .battery:
            return "Charge the Mac above the configured floor before using Bag Mode."
        ...
        case .unsupportedHardware:
            return "Use normal sleep prevention on this Mac until Bag Mode support is validated."
        case .coverageInsufficient:
